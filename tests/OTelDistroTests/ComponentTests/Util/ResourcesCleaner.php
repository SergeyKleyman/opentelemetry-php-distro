 ...
 * @phpstan-type ProcessTerminateData array{'dbg_desc': string, 'subTree': ProcessesInfo}
 * @phpstan-type PidToProcessTerminateData array<Pid, ProcessTerminateData>
 */
final class ResourcesCleaner extends TestInfraHttpServerProcessBase
{
    ...
    public const UPDATE_PROCESS_TO_TERMINATE_REGISTRATION_URI_PATH = TestInfraHttpServerProcessBase::BASE_URI_PATH . 'update_process_to_terminate_registration';
    ...
    public const TERMINATE_RELATED_PROCESSES_URI_PATH = TestInfraHttpServerProcessBase::BASE_URI_PATH . 'terminate_related_processes';
    ...
    protected function processRequest(ServerRequestInterface $request): ?ResponseInterface
    {
        switch ($request->getUri()->getPath()) {
            case self::REGISTER_PROCESS_TO_TERMINATE_URI_PATH:
                $this->handleProcessToTerminateRegistration($request, registerOrUpdate: true);
                break;
            case self::UPDATE_PROCESS_TO_TERMINATE_REGISTRATION_URI_PATH:
                $this->handleProcessToTerminateRegistration($request, registerOrUpdate: false);
                break;
            case self::REGISTER_FILE_TO_DELETE_URI_PATH:
                $this->registerFileToDelete($request);
                break;
            case self::TERMINATE_RELATED_PROCESSES_URI_PATH:
                $this->terminateRelatedProcesses($request);
                break;
    ...
    protected function handleProcessToTerminateRegistration(ServerRequestInterface $request, bool $registerOrUpdate): void
    {
    ...
    private function terminateProcessesFromSet(string $dbgProcessesSetDesc, array $pidToDbgDesc): bool
    {
        ...
        public const MAX_WAIT_AFTER_KILL_SECONDS = HttpClientUtilForTests::TIMEOUT_SECONDS * 3/4;
        public const MAX_WAIT_FOR_PROCESSES_TO_EXIT_AFTER_FORCE_KILL_SECONDS = 10;
