...
if [ -z "${OTEL_PHP_TESTS_EXTERNAL_SERVICES_ENV_VARS_ARE_SET+x}" ] || [ "${OTEL_PHP_TESTS_EXTERNAL_SERVICES_ENV_VARS_ARE_SET}" != "true" ] ; then
    export OTEL_PHP_TESTS_DOCKER_NETWORK=elastic-otel-php-tests-component-network

    export OTEL_PHP_TESTS_MYSQL_HOST=elastic-otel-php-tests-component-mysql
    export OTEL_PHP_TESTS_MYSQL_PORT=3306
    export OTEL_PHP_TESTS_MYSQL_USER=root
    export OTEL_PHP_TESTS_MYSQL_PASSWORD=elastic-otel-php-tests-component-mysql-password
